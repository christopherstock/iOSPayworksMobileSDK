...
typedef NS_OPTIONS(NSUInteger, MPPaymentAccessoryFeature) {
    ...
    /** Capable of deferred authorization transactions */
    ...
    /** Has battery */
    MPPaymentAccessoryFeatureBattery = 1 << 14,

    /** Supports SCA Online PIN workflow */
    MPPaymentAccessoryFeatureSCAOnlinePIN = 1 << 15,

    /** Supports SCA Second Tap with CDCVM Device workflow */
    MPPaymentAccessoryFeatureSCASecondTap = 1 << 16
};
 ...
 * @since 2.0.0
 */
typedef void (^MPPaymentAccessoryDisplayTextSuccess)(MPAccessory *_Nonnull accessory, NSArray<NSString *> *_Nonnull text);
/**
 * Failure handler for displaying text on the accessory.
 * @param error Error that occurred during the process
 * @since 2.0.0
 */
typedef void (^MPPaymentAccessoryDisplayTextFailure)(MPAccessory *_Nonnull accessory, NSArray<NSString *> *_Nonnull text, NSError *_Nonnull error);
...
/**
 * The language to use for displaying infos and prompts for the merchant.
 * @since 2.0.0
 */
 ...
 * @since 2.0.0
 */
@property (strong, readwrite, nonatomic, nonnull) NSArray<NSString *> *idleScreenText;
 ...
 * @deprecated 2.11.0
 */
- (void)displayText:(nonnull NSArray<NSString *> *)text
            success:(nonnull MPPaymentAccessoryDisplayTextSuccess)success
            failure:(nonnull MPPaymentAccessoryDisplayTextFailure)failure DEPRECATED_MSG_ATTRIBUTE("Use MPAccessoryComponentInteraction.");
 ...
 * @deprecated 2.11.0
 */
- (void)displayIdleScreenWithSuccess:(nullable MPPaymentAccessoryDisplayIdleScreenSuccess)success
                             failure:(nullable MPPaymentAccessoryDisplayIdleScreenFailure)failure DEPRECATED_MSG_ATTRIBUTE("Use MPAccessoryComponentInteraction.");
